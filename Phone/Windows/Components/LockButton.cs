...
internal static class LockButton
{
    public static bool Draw(Vector2 center, float radius, FontAwesomeIcon icon, bool active, PhoneTheme theme) =>
        Draw(ImGui.GetWindowDrawList(), center, radius, icon, active, theme);

    public static bool Draw(ImDrawListPtr drawList, Vector2 center, float radius, FontAwesomeIcon icon, bool active,
        PhoneTheme theme)
    {
        ...
        drawList.AddCircleFilled(center, radius, ImGui.GetColorU32(background), 32);
        var ink = active || hovered ? theme.TextStrong : theme.TextMuted;
        ProgressRing.CenterIcon(drawList, center, icon, ink, radius * 1.02f);
        if (hovered)
        {
