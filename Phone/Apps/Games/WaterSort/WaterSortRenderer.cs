...
namespace VideoSyncPrototype.Phone.Apps.Games.WaterSort;
...
internal sealed class WaterSortRenderer
{
    ...
    private void DrawTube(ImDrawListPtr drawList, WaterSortBoard board, int tube, Rect rect, float scale,
        PhoneTheme theme, bool lifted)
    {
        ...
        drawList.AddRectFilled(rect.Min, rect.Max, ImGui.GetColorU32(new Vector4(1f, 1f, 1f, 0.05f)), rounding,
        ...
        for (var level = 0; level < board.Count(tube); level++)
        {
            ...
            drawList.AddRectFilled(bandMin, bandMax, ImGui.GetColorU32(ColorOf(color)),
                level == 0 ? rounding - wall : 0f, flags);
        }
