...
namespace VideoSyncPrototype.Phone.Apps.Games.GemSwap;
...
internal sealed class GemSwapRenderer
{
        ...
        int color, float rounding, float half, float scale, PhoneTheme theme, float entrance)
    {
        ...
        else
        {
            ...
            Typography.DrawCentered(center, GemSymbols[color], ink with { W = ink.W * alpha }, gemHalf / (20f * scale),
                FontWeight.SemiBold);
        }
    ...
    private void DrawSpecial(ImDrawListPtr drawList, Vector2 center, float gemHalf, GemSpecial special, float alpha,
        float scale)
    {
        ...
        switch (special)
        {
            case GemSpecial.LineHorizontal:
                drawList.AddLine(new Vector2(center.X - gemHalf * 0.7f, center.Y - gemHalf * 0.32f),
                    new Vector2(center.X + gemHalf * 0.7f, center.Y - gemHalf * 0.32f), white, thickness);
                drawList.AddLine(new Vector2(center.X - gemHalf * 0.7f, center.Y + gemHalf * 0.32f),
                    new Vector2(center.X + gemHalf * 0.7f, center.Y + gemHalf * 0.32f), white, thickness);
                break;
            case GemSpecial.LineVertical:
                drawList.AddLine(new Vector2(center.X - gemHalf * 0.32f, center.Y - gemHalf * 0.7f),
                    new Vector2(center.X - gemHalf * 0.32f, center.Y + gemHalf * 0.7f), white, thickness);
                drawList.AddLine(new Vector2(center.X + gemHalf * 0.32f, center.Y - gemHalf * 0.7f),
                    new Vector2(center.X + gemHalf * 0.32f, center.Y + gemHalf * 0.7f), white, thickness);
                break;
