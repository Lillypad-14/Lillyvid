...
namespace VideoSyncPrototype.Phone.Apps.Games.Pairs;
...
internal sealed class PairsRenderer
{
    ...
    public void DrawCard(Rect cell, int symbolIndex, CardState state, float flipProgress, float glow, float shakeX,
        bool hovered, PhoneTheme theme, float scale)
    {
    ...
    private void DrawBack(ImDrawListPtr drawList, Vector2 min, Vector2 max, float rounding, bool hovered,
        PhoneTheme theme, float scale)
    {
        ...
        if (hovered)
        {
            ...
            Squircle.Stroke(drawList, min, max, rounding, ImGui.GetColorU32(theme.Accent with { W = 0.6f }),
                1.5f * scale);
        }
        else
        {
    ...
    private void DrawFace(ImDrawListPtr drawList, Vector2 min, Vector2 max, Vector2 center, float rounding,
        int symbolIndex, float glow, float scale)
    {
        ...
        Squircle.Stroke(drawList, min, max, rounding, ImGui.GetColorU32(color with { W = 0.7f + glow * 0.3f }),
        ...
        Typography.DrawCentered(center, Symbols[symbolIndex % Symbols.Length], symbolColor, symbolScale,
            FontWeight.SemiBold);
    }
}
