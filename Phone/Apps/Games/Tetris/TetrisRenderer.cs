...
namespace VideoSyncPrototype.Phone.Apps.Games.Tetris;
...
internal sealed class TetrisRenderer
{
    ...
    private static void DrawSlotFrame(ImDrawListPtr drawList, Rect rect, PhoneTheme theme, Vector4 accent,
        string caption, bool hovered, float scale)
    {
        ...
        Squircle.Stroke(drawList, rect.Min, rect.Max, 18f * scale,
            ImGui.GetColorU32(GamePalette.Lighten(accent, 0.32f) with { W = hovered ? 0.75f : 0.35f }), 1f * scale);
        Typography.DrawCentered(new Vector2(rect.Center.X, rect.Min.Y + 12f * scale), caption, theme.TextMuted,
            TextStyles.Caption2);
    }
    ...
    private static void DrawActivePiece(ImDrawListPtr drawList, TetrisBoard board, GameGrid grid, int boardY,
    ...
    private static void DrawCell(ImDrawListPtr drawList, GameGrid grid, int column, int row, Vector4 color, float alpha,
        float scale)
    {
    ...
    private static void DrawPiecePreview(ImDrawListPtr drawList, Rect rect, TetrisPieceKind kind, float scale)
    {
        ...
        var origin = new Vector2(rect.Center.X - previewWidth * 0.5f,
