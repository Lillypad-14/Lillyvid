...
namespace VideoSyncPrototype.Phone.Apps.Games.Sweeper;

internal sealed class SweeperRenderer
{
    private static readonly Vector4[] NumberColors =
    {
        new(0.40f, 0.68f, 0.98f, 1f), new(0.46f, 0.86f, 0.66f, 1f), new(0.95f, 0.45f, 0.50f, 1f),
        new(0.75f, 0.50f, 0.95f, 1f), new(0.95f, 0.62f, 0.30f, 1f), new(0.40f, 0.78f, 0.82f, 1f),
        new(0.86f, 0.86f, 0.90f, 1f), new(0.62f, 0.62f, 0.68f, 1f),
    };

    private static readonly Vector4 MineColor = new(0.93f, 0.42f, 0.50f, 1f);

    public void Draw(SweeperBoard board, GameGrid grid, int hoveredIndex, float[] flagAnim, PhoneTheme theme,
        float scale)
    {
        ...
        for (var row = 0; row < board.Rows; row++)
        {
            for (var column = 0; column < board.Columns; column++)
            {
                ...
                {
                    DrawCovered(drawList, board, index, cell, rounding, index == hoveredIndex, flagAnim[index], theme,
                        scale);
                }
            }
        }
    ...
    private void DrawRevealed(ImDrawListPtr drawList, SweeperBoard board, int index, Rect cell, float rounding,
        PhoneTheme theme, float scale)
    {
        ...
        if (adjacent > 0)
        {
            Typography.DrawCentered(cell.Center, GameNumber.Label(adjacent), NumberColors[adjacent - 1], 1.2f,
                FontWeight.Bold);
        }
    }

    private void DrawCovered(ImDrawListPtr drawList, SweeperBoard board, int index, Rect cell, float rounding,
        bool hovered, float flagPop, PhoneTheme theme, float scale)
    {
    ...
    private void DrawFlag(ImDrawListPtr drawList, Vector2 center, float size, Vector4 accent)
    {
        ...
        drawList.AddLine(poleTop, poleBottom, ImGui.GetColorU32(new Vector4(0.85f, 0.85f, 0.88f, 1f)),
            MathF.Max(1.5f, size * 0.18f));
