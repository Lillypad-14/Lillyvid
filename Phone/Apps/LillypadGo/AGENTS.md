...
| Encounters | `EncounterService.cs` | Step accrual → rolls a wild from the biome table. Also announces a living region Alpha on zone entry. |
| Region Alphas | `Alphas.cs` | One uncatchable boss per region: catalog (species/level/territory/lair map-coords/trait/themed weather/spoils), trait presentation, loot rolls, respawn window, and lair geometry (world position via `MapCoordinateConverter`, in-game map flag via `MapLinkPayload`, `ChallengeRadius` proximity gate). Battle hooks live in `Battle.cs` (`IsAlphaBattle`, `Wild.Trait`); the themed `BattleWeather` is passed at engage time and rules the whole fight. The glowing aura is `DrawAlphaAura` in `LillypadGoApp.Shared.cs` (shared by battle/map/dex; `strength` boosts it for the large battle sprite). Defeat history in `LillypadGoState` (live player position is transient there too), the lair card in `LillypadGoApp.Map.cs`, the ALPHAS folder tab in `LillypadGoApp.Dex.cs`. |
| Persistence | `LillypadGoState.cs` | `Plugin.LillypadGo`; saves to `lillypadgo.json`. Call `State.Save()` after mutations. |
| Overworld follower + world battles | `FollowerRenderer.cs`, `WorldBattleStage.cs` | The lead Pokémon follows the player in the actual game world, and phone battles are mirrored as an in-world stage (battlers, traced move fx, capture balls, damage popups) IN TANDEM with the phone. `DrawBattle` publishes a per-frame `WorldBattleStage.Snapshot`; `FollowerRenderer` (driven from `MainWindow.DrawWorldSurfaceOverlay`) renders it via the native scene-pass `NativeWorldSprite` billboards (`Rendering/NativeTestRenderer.cs`). Toggles: `State.FollowerEnabled`, `State.WorldBattlesEnabled` (Options → Overworld). |
| Immerse mode (opt-in) | `LillypadGoApp.Immersive.cs` | Play without the phone: wild spawns roam the world (click → Engage), battles run from an on-screen hotbar (keys 1–4 moves w/ app tooltips, 5 Bag, 6 Swap, 7 Run, 8 Phone; keys captured only during battles via `IKeyState`, real hotbars untouched). `TickBattleHeadless` mirrors `DrawBattle`'s logic half so the battle advances with the phone closed — keep its capture/send-out timings in sync with `DrawBattle`. Entry: `LillypadGoApp.Instance.DrawImmersiveOverlay()` from `MainWindow.DrawWorldSurfaceOverlay`. Toggle: `State.ImmersiveModeEnabled` (default off). |
